...
class CategoriaRepasoAnimales extends StatelessWidget {
   
  const CategoriaRepasoAnimales({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return  Scaffold(
      body: Container(
       ...
       height: size.height,
       width: size.width,
       child: FutureBuilder<List<Animales>>(
        future: MazahuaDataBase.instance.getAllAnimales(),
         builder: (BuildContext context, AsyncSnapshot<List<Animales>> snapshot) {
            if(snapshot.hasData){
              return Column(
          ...
          return GestureDetector(
            onTap: ()async {
              Soundpool pool = Soundpool(streamType: StreamType.notification);
                int soundId = await rootBundle.load(animal.getAudio).then((ByteData soundData) {
            ...
            child: Card(
              ...
              child: Column(
                ...
                children: [
                  Image.asset(animal.getImage, height: 90,),
                  Text(
                    animal.nombre_es.toUpperCase(),
